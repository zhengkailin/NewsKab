...
<div class="container headinfo">
    ...
    <div class="locaiton"><a href="/indexlist">首页</a>&gt;&gt;创业资讯</div>
</div>
<div class="container mianinfo">
    <ul>
        <c:forEach items="${page.datas}" var="e">
        <li><a href="/entrepreneurshiplById/${e.id}">· ${e.title}</a>
            <span class='pull-right'>${e.creationDate}</span>
        </li>
        </c:forEach>
    </ul>
</div>
...
//            $("#formList").submit();
            //--------------------------------------------------
            //         访问路径的方式
            location.href="${ctx}/entrepreneurshiplist?currPageNo="+p;
        }
    });
</script>
